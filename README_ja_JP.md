...
* [🛠️ 開発ガイド](#️-開発ガイド)
* [❓ FAQ](#-faq)
  * [Wellspringはどのようにデータを保存しますか？](#wellspringはどのようにデータを保存しますか)
  * [サードパーティの同期ディスクを介したデータ同期をサポートしていますか？](#サードパーティの同期ディスクを介したデータ同期をサポートしていますか)
  * [Wellspringはオープンソースですか？](#wellspringはオープンソースですか)
...
Wellspringは、プライバシーを最優先とする個人の知識管理システムであり、細かいブロックレベルの参照とMarkdown WYSIWYGをサポートしています。

詳細については、[Wellspring英語ディスカッションフォーラム](https://liuyun.io)をご覧ください。
...
* [Wellspringの開発計画と進捗](https://github.com/orgs/siyuan-note/projects/1)
* [Wellspringの変更履歴](CHANGELOG.md)
...
サーバーでWellspringを提供する最も簡単な方法は、Dockerを使用してデプロイすることです。
...
Docker Composeを使用してWellspringを実行するユーザー向けに、環境変数 `PUID` と `PGID` を使用してユーザーとグループのIDをカスタマイズできます。以下はDocker Composeの設定例です：
...
### Wellspringはオープンソースですか？

Wellspringは完全にオープンソースであり、貢献を歓迎します：
...
* デスクトップでインストールパッケージを使用してインストールした場合は、<kbd>設定</kbd> - <kbd>情報</kbd> - <kbd>自動的に更新インストールパッケージをダウンロード</kbd> オプションを開くことができます。これにより、Wellspringは最新バージョンのインストールパッケージを自動的にダウンロードし、インストールを促します
...
Wellspringの誕生は、多くのオープンソースプロジェクトと貢献者なしでは実現できませんでした。プロジェクトのソースコード kernel/go.mod、app/package.json、およびプロジェクトのホームページをご覧ください。

Wellspringの成長は、ユーザーのフィードバックとプロモーションなしでは実現できませんでした。Wellspringへのすべての支援に感謝します ❤️

### 貢献者

私たちに参加し、一緒にWellspringにコードを貢献することを歓迎します。
